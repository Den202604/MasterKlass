...
Процедура СписокНоменклатурыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ЧтоПередаем = Новый Структура;
	ЧтоПередаем.Вставить("Номенклатура", ДанныеСтроки.Номенклатура);
	ЧтоПередаем.Вставить("Цена", ДанныеСтроки.Цена);
	
	Массив = Новый Массив;
	Массив.Добавить(ЧтоПередаем);
...
Процедура ПозицииПродажиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ПереданноеЗначение = ПараметрыПеретаскивания.Значение[0];
	
	Если ТипЗнч(ПереданноеЗначение) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	СтандартнаяОбработка = Ложь;
	
	ДобавитьУникальнуюПозицию(ПереданноеЗначение.Номенклатура, ПереданноеЗначение.Цена)
КонецПроцедуры
